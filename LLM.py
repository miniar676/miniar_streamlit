...
# CONFIGURATION
# ======================================
DATASET_PATH = "C:/Users/MSI/Desktop/IA_project/finale_translitteration_corrigee__corrigee.jsonl"
...
if st.session_state.person_data:
    ...
    caption = person.get("caption", "N/A")
    caption_translit = person.get("caption_translit", "N/A")

    st.markdown(f"**🗣️ Caption :** {caption}")
    st.markdown(f"**🔤 Caption translittéré :** {caption_translit}")
    ...
    if st.button("✨ Générer un résumé LLM"):
        with st.spinner("⏳ Génération du résumé avec Mistral..."):
            summary = generate_person_report(person)
